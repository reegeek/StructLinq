...
namespace StructLinq.Benchmark
{
    ...
    public class TakeWhileOnArray
    {
        ...
        {
            var sum = 0;
            foreach (var i in array.ToStructEnumerable().TakeWhile(x=> x > 5000))
            {
                sum += i;
            }
        ...
        {
            var sum = 0;
            foreach (var i in array.ToStructEnumerable().TakeWhile(x=> x > 5000, x=>x))
            {
                sum += i;
            }
        ...
        public int StructLinqFunctionZeroAlloc()
        {
            ...
            foreach (var i in array.ToStructEnumerable().TakeWhile(predicate, x=>x))
            {
                sum += i;
            }
