...
namespace StructLinq.Benchmark
{
    ...
    public class IntersectOnBigStruct
    {
        ...
        {
            var sum = 0;
            foreach (var i in array1.ToRefStructEnumerable().Intersect(array2.ToRefStructEnumerable(), x => x, x => x))
            {
                sum += i.Element;
            }
            return sum;
        ...
        public int RefStructLinqWithComparer()
        {
            ...
            foreach (var i in array1.ToRefStructEnumerable().Intersect(array2.ToRefStructEnumerable(), comparer, x => x, x => x))
            {
                sum += i.Element;
            }
            return sum;
