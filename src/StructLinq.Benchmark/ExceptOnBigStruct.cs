...
namespace StructLinq.Benchmark
{
    ...
    public class ExceptOnBigStruct
    {
        ...
        {
            var sum = 0;
            foreach (var i in array1.ToRefStructEnumerable().Except(array2.ToRefStructEnumerable(), x => x, x => x))
            {
                sum += i.Element;
            }
            return sum;
        ...
        public int RefStructLinqWithComparer()
        {
            ...
            foreach (var i in array1.ToRefStructEnumerable().Except(array2.ToRefStructEnumerable(), comparer, x => x, x => x))
            {
                sum += i.Element;
            }
            return sum;
